...
def darken_color(color, iterations):
    ...
    for _ in range(iterations):
        # Reduce color components by step value
        red = max(0, red - step)
    ...
    # Return the final darkened color in hexadecimal format
...
class Shell(commands.Cog):
    ...
    @shell_group.command()
    async def baltop(self, message):
        db = DB()
        db.connect()
        db.cursor.execute(f'SELECT shells.user, discord_links.uuid, shells.bal FROM shells JOIN discord_links ON shells.user = discord_links.discord_id;')
        ...
        for member in data:
            ...
            for player in playerdata:
                if member['uuid'] == player['uuid']:
                    found = True
                    shelldata.append({'name': member['name'], 'rank': member['rank'], 'shells': player['bal']})
                else:
                    pass
            if not found:
                shelldata.append({'name': member['name'], 'rank': member['rank'], 'shells': 0})
        ...
        for page in range(page_num):
            ...
            d.fontmode = '1'
            page_playerdata = shelldata[(10 * page):10 + (10 * page)]
            for p, player in enumerate(page_playerdata):
                ...
                _, _, w, h = d.textbbox((0, 0), "{:,}".format(player["shells"]), font=gameFont)
                if i == 1:
                    widest = w
                addLine(f'&f{"{:,}".format(player["shells"])}', d, gameFont, img.width - 40 - w, p * 36 + 9)
