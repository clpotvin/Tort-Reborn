...
class UpdateMemberData(commands.Cog):
    ...
    async def update_member_data(self):
        ...
        try:
            ...
            if not (self.cold_start and not self.member_file_exists):
                if joined or left:
                    mem_tracking_channel = self.client.get_channel(LOG_CHANNEL)
                    embed = discord.Embed(
                        title="Guild Membership Changes",
                        timestamp=now,
                        color=0xFFA500
                    )
                    def add_chunked(field_name, names, name_map):
                        bolded = [f"**{name_map[u]}**" for u in names]
                        chunk = ""
                        for nm in bolded:
                            piece = nm if not chunk else ", " + nm
                    ...
                    if joined:
                        add_chunked("Joined", joined, curr_map)
                    if left:
                        add_chunked("Left", left, prev_map)
                    if mem_tracking_channel:
                        await mem_tracking_channel.send(embed=embed)
